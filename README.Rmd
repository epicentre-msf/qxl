...
A call to `qxl` only writes one sheet at a time. To create a workbook with
multiple sheets we need to pass an existing workbook to the `wb` argument, which
can be done for example with piping.
